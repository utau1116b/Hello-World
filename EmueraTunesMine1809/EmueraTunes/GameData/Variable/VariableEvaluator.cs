...
namespace MinorShift.Emuera.GameData.Variable
{
	internal sealed class VariableEvaluator : IDisposable
	{
		...
		public void SetValueAll(FixedVariableTerm p, Int64 srcValue, int start, int end)
		{
            ...
            //    {
            //        long[,] array = chara.DataIntegerArray2D[p.Identifier.VarCodeInt];
            ...
            //        long[] array = chara.DataIntegerArray[p.Identifier.VarCodeInt];
            ...
            //    else
            //    {
            //        chara.DataInteger[p.Identifier.VarCodeInt] = srcValue;
            //        return;
            //    }
            //}
            //else if (!p.Identifier.IsArray1D && !p.Identifier.IsArray2D && !p.Identifier.IsArray3D)
            //{
            //    varData.DataInteger[p.Identifier.VarCodeInt] = srcValue;
            //    return;
            //}
            else
            {
            ...
            //{
            //    //long[, ,] a3 = varData.DataIntegerArray3D[p.Identifier.VarCodeInt];
            ...
            //{
            //    //long[,] a2 = varData.DataIntegerArray2D[p.Identifier.VarCodeInt];
		...
		public void SetValueAllEachChara(FixedVariableTerm p, SingleTerm index, Int64 srcValue, int start, int end)
		{
			...
			for (int i = start; i < end; i++)
			{
				...
				if (p.Identifier.IsArray1D)
					chara.DataIntegerArray[p.Identifier.VarCodeInt][indexNum] = srcValue;
				else
					chara.DataInteger[p.Identifier.VarCodeInt] = srcValue;
			}
		}
        ...
        public Int64 GetInRangeArrayChara(FixedVariableTerm p, Int64 min, Int64 max, Int64 start, Int64 end)
        {
            ...
            if (p.Identifier.IsArray2D)
            {
                for (int i = (int)start; i < (int)end; i++)
                {
                    chara = varData.CharacterList[i];
                    Int64[,] array = chara.DataIntegerArray2D[p.Identifier.VarCodeInt];
                    if (array[p.Index2, p.Index3] >= min && array[p.Index2, p.Index3] < max)
                        ret++;
                }
            }
			else if (p.Identifier.IsArray1D)
			{
				for (int i = (int)start; i < (int)end; i++)
				{
					chara = varData.CharacterList[i];
					Int64[] array = chara.DataIntegerArray[p.Identifier.VarCodeInt];
					if (array[p.Index2] >= min && array[p.Index2] < max)
						ret++;
				}
			}
			else
			{
				...
				{
					chara = varData.CharacterList[i];
					if (chara.DataInteger[p.Identifier.VarCodeInt] >= min && chara.DataInteger[p.Identifier.VarCodeInt] < max)
						ret++;
				}
			}
			return ret;
		...
		{
			return string.Format("{0}var{1:00}.sav", Config.SavDir, index);
		}
		private string getSaveDataPathC(int index)
		{
			return string.Format("{0}chara{1:00}.sav", Config.SavDir, index);
		}
		...
		public void LoadVariable(int saveIndex)
		{
			string filepath = getSaveDataPathV(saveIndex);
