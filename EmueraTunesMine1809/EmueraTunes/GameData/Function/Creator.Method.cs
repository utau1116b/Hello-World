    ...
    internal static partial class FunctionMethodCreator
    {
        ...
        private sealed class GetcharaMethod : FunctionMethod
        {
			...
			public override Int64 GetIntValue(ExpressionMediator exm, IOperandTerm[] arguments)
            {
                ...
                if (CheckSp)
                {
                    chara = exm.VEvaluator.GetChara(integer, false);
                    if (chara != -1)
                        return chara;
                    else
                        return exm.VEvaluator.GetChara(integer, true);
                }
                else
                    return exm.VEvaluator.GetChara(integer, false);
            }
        }
        ...
        private sealed class CheckdataMethod : FunctionMethod
        {
            ...
            public override Int64 GetIntValue(ExpressionMediator exm, IOperandTerm[] arguments)
            {
				...
				if (type != EraSaveFileType.Normal)
					throw new CodeEE("未実装の関数です");
        ...
        private sealed class MouseSkipMethod : FunctionMethod
        {
            public MouseSkipMethod()
            {
                ReturnType = typeof(Int64);
                argumentTypeArray = new Type[] { };
                CanRestructure = false;
            ...
            {
                return GlobalStatic.Console.inMouseSkip ? 1L : 0L;
            }
        }
        ...
        private sealed class GetnumMethod : FunctionMethod
        {
            ...
            public override Int64 GetIntValue(ExpressionMediator exm, IOperandTerm[] arguments)
            {
                ...
                try
                {
                    ret = exm.VEvaluator.Constant.KeywordToInteger(varCode, key, -1);
                }
                catch
                {
                    ret = -1;
