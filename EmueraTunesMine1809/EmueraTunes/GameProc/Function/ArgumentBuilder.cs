	...
	internal static partial class ArgumentParser
	{
        ...
        private sealed class SP_SORT_ARRAY_ArgumentBuilder : ArgumentBuilder
        {
            public override Argument CreateArgument(InstructionLine line, ExpressionMediator exm)
            {
                ...
                { warn("第１引数が読み込み専用の変数です", line, 2, false); return null; }
			...
			public override Argument CreateArgument(InstructionLine line, ExpressionMediator exm)
			{
                ...
                for (int i = 0; i < termList.Count; i++)
                {
                    if (termList[i] is SingleTerm)
                    {
                        ...
                        if ((bit < 0) || (bit > 63))
                        {
                            warn("第" + Strings.StrConv((i+2).ToString(), VbStrConv.Wide, Config.Language) + "引数が範囲(０～６３)を超えています", line, 2, false);
                            return null;
                        }
                    }
                }
		...
		private sealed class SP_SAVEVAR_ArgumentBuilder : ArgumentBuilder
		{
			...
			{
				argumentTypeArray = new Type[] { typeof(Int64), typeof(string), typeof(void)};
				argAny = true;
				minArg = 3;
			}
			public override Argument CreateArgument(InstructionLine line, ExpressionMediator exm)
