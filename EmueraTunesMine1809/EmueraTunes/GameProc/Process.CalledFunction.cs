		...
		public UserDefinedFunctionArgument ConvertArg(IOperandTerm[] srcArgs, out string errMes)
		{
			...
			for (int i = 0; i < func.Arg.Length; i++)
			{
				...
				if (destArg.Identifier.IsReference)//参照渡しの場合
				{
					...
					if (vTerm == null|| vTerm.Identifier.Readonly || vTerm.Identifier.IsCalc)
					{
						errMes = "\"@" + func.LabelName + "\"の" + (i + 1).ToString() + "番目の引数は参照渡しのための配列変数でなければなりません";
						return null;
					}
					//変数の型と次元数が一致すればいい。キャラ型については後に判断
					VariableCode destCodeFlag = destArg.Identifier.Code &
						(VariableCode.__ARRAY_1D__ | VariableCode.__ARRAY_2D__ | VariableCode.__ARRAY_3D__ | VariableCode.__INTEGER__ | VariableCode.__STRING__);
					VariableCode srcCodeFlag = vTerm.Identifier.Code &
						(VariableCode.__ARRAY_1D__ | VariableCode.__ARRAY_2D__ | VariableCode.__ARRAY_3D__ | VariableCode.__INTEGER__ | VariableCode.__STRING__);
					if (destCodeFlag != srcCodeFlag)
					{
						errMes = "\"@" + func.LabelName + "\"の" + (i + 1).ToString() + "番目の引数の型が一致しません";
						return null;
					}
				}
				else if (term == null)//引数が省略されたとき
				{
					...
					{
						errMes = "\"@" + func.LabelName + "\"の" + (i + 1).ToString() + "番目の引数は省略できません";
						return null;
					}
				}
				else if (term.GetOperandType() != destArg.GetOperandType())
				{
					...
					else
					{
						...
						{
							errMes = "\"@" + func.LabelName + "\"の" + (i + 1).ToString() + "番目の引数を整数型から文字列型に変換できません";
							return null;
						}
						if (tostrMethod == null)
