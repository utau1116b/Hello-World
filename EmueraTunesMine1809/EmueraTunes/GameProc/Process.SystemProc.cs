...
namespace MinorShift.Emuera.GameProc
{
	internal sealed partial class Process
	{
		...
		void beginTitle()
		{
			...
			{
				console.PrintLine("ファイル解析終了：Analysis.logに出力します");
				console.outputLog("Analysis.log");
				console.noOutputLog = true;
				console.PrintLine("エンターキーもしくはクリックで終了します");
			...
			{
				console.PrintLine("ERBコードに解釈不可能な行があるためEmueraを終了します");
				console.PrintLine("emuera.logにログを出力します");
			...
			console.PrintBar();
			console.NewLine();
			console.PrintCenter(gamebase.ScriptTitle);
			if (gamebase.ScriptVersion != 0)
				console.PrintCenter(gamebase.ScriptVersionText);
			console.PrintCenter(gamebase.ScriptAutherName);
			console.PrintCenter("(" + gamebase.ScriptYear + ")");
			console.NewLine();
			console.PrintCenter(gamebase.ScriptDetail);
			console.PrintBar();
			console.NewLine();
			console.PrintLine("[0] " + Config.TitleMenuString0);
			console.PrintLine("[1] " + Config.TitleMenuString1);
			openingInput();
			return;
		}
		...
		void trainWaitInput()
		{
			...
			else
			{
				...
				console.PrintLine(string.Format("＜コマンド連続実行：{0}/{1}＞", count, coms.Count));
			}
			...
			{//されていない。
				if (isCTrain)
					console.PrintLine("コマンドを実行できませんでした");
		...
		void saveGameWaitInput()
		{
			...
			if (available)
			{
				console.PrintLine("既にデータが存在します。上書きしますか？");
		...
		{
			//EraDataResult result = vEvaluator.checkData(saveIndex);
